    ...
    <section className="space-y-8">
      ...
      <div className="space-y-6">
        {paginatedResults.map((post) => (
          <article
            key={post.id}
            className="rounded-3xl border border-white/10 bg-neutral-900/50 p-6 transition hover:border-white/30"
          >
            ...
            <Link href={`/posts/${post.slug}`} className="mt-4 block text-2xl font-semibold">
              {post.title}
            </Link>
            {post.excerpt ? (
